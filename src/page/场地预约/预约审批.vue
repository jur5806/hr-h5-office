<template>
  <div class="warp page1">
    <v-header :goBack="true" headTitle="预约审批" :serviceGet="true">
    ...
    <v-scroll ref="my_scroller" class="service-main-box7 scroll" :on-refresh="refresh" :on-infinite="infinite">
      ...
      <div v-else>
        <div class="g-reservation" v-for="item in appointmentRecordList" @click="$router.push('/appointmentRecord?appId='+item.id+'&type=1&checkState='+checkState)">
          ...
          <div class="flex-box g-roomdetails">
            ...
            <div class="g-roomdesc">
              <div class="flex-box g-yuyuestate">
                <h4 class="ellipsis">{{item.fieldName}}</h4>
                <span v-if="item.appState==3" class="s-danger">审批驳回</span>
                <span v-else-if="item.appState==1 && navstate == 0">待审批</span>
                <span v-else-if="item.appState==7 && navstate == 0">已使用</span>
                <span v-else-if="item.appState==5" class="s-fail">已取消</span>
                <span v-else class="s-success">审批成功</span>

              </div>
              <p class="ellipsis g-capacity">预约人：{{item.appName}}</p>
              <div class="g-yuyuetime">
                预约时间：{{new Date(item.currentDate).format('yyyy/MM/dd')}}
                <span v-if="item.appHourIntervalVOList.length == 1">{{item.appHourIntervalVOList[0].appStartTime}} -{{item.appHourIntervalVOList[0].appEndTime}}</span>
                <span v-if="item.appHourIntervalVOList.length > 1">{{item.appHourIntervalVOList[0].appStartTime}} -{{item.appHourIntervalVOList[0].appEndTime}} 等</span>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refresh (done) {
        getData.fieldAppointmentList(1, this.checkState).then(res => {
      ...
      infinite (done) {
        ...
        getData.fieldAppointmentList(this.pageIndex, this.checkState).then(res => {
          this.isLoading = false
          if (res.data.code !== 1) {
            done('error')
            return
