<template>
  <div class="warp page1 g-page-grey">
    <v-header :goBack="true" headTitle="服务申请">
    </v-header>
    ...
    <!-- <div class="g-enterprise-bg" style="flex:1"> -->
      ...
      <v-scroll class="scroll" :class="isData === 2 ? 'main-box28' : 'main-box29'" ref="my_scroller" :on-refresh="refresh" :on-infinite="infinite" style="flex:1">
          ...
          <ul style="margin-top: .2rem;">
            <li class="g-enter-item" v-for="(item, index) in list" :key="index" @click.stop="$router.push('/enterpriseServiceDetails?odId=' + item.odId)">
              <div class="flex-box g-enter-top">
                ...
                <p :class="{
                  'bg-red': item.odStatus === 1 || item.odStatus === 2,
                  'bg-green': item.odStatus === 4,
                }">{{getStatus(item.odStatus)}}</p>
              </div>
              <p class="p1 ellipsis2">{{item.odDescription}}</p>
              <div class="flex-box g-enter-bottom">
                <div class="flex-box">
                  <img class="img1" src="../../assets/enterpriseImg/handle-time.png" alt="">
                  <p class="p-times">{{new Date(item.odCreateTime).format('yyyy-MM-dd hh:mm:ss')}} 提交</p>
                </div>
        ...
        <div class="g-emptys-enter" v-if="isData === 3">
          <div class="g-conboxs">
            ...
            <div class="g-enterprise-footer g-bots">
              <p @click.stop="$router.push('/chooseEnterpriseService?type=3')">提交服务申请</p>
            </div>
          </div>
        </div>
      </v-scroll>
