<template>
  <header :class="{'top-searchbar':true, 'flex-box':true,'c-login-bg':loginBg}" v-if="$store.state.platform !== 'iOS'&&$store.state.platform !== 'Android'">
    <slot name="back"></slot>
...
<script>
  ...
  export default {
    ...
    methods: {
    	back () {
    	  if (this.$route.query.isPush === '1') {
    	    this.$router.replace('/progressOfPropertyServices?isPush=2')
        } else if (this.$route.query.isPush === '2') {
...
<style scoped lang="scss">
  ...
  .top-searchbar{
    ...
    z-index: 200;
    height: .8rem;
    background: url("../assets/workorderimg/workorder-img.png") no-repeat;
